$(document).ready(function(e) {
    ...
    $("body").on("click",".eliminarSancion",function (e) {
        var sancionID = $(this).data("id");
        txt='¿Estás seguro que deseas Eliminar la sanción?';

        let fd  = {"sancionID":sancionID};
        Swal.fire({
            title: '',
            text: txt,
            icon: "question",
    ...
    function ajaxCambiarEstado(fd){
        $.ajax({
            url: BASE_URL + "Incidencias/ajax_eliminarActa",
            cache: false,
            type: 'post',
            dataType: 'json',
            data:fd
            ...
            if(data.code === 1) {
                $.toast({
                    text:'Se ha eliminado la sanción.',
                    icon: "success",
                    loader: true,
                    loaderBg: '#c6c372',
                    position: 'top-right',
