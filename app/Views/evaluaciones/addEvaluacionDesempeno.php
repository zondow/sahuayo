...
<style>
    .new_friend_list {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); /* Ajusta el tamaño de los elementos automáticamente */
    gap: 30px;
...
<script>
    $(document).ready(function () {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { cuestionarioID: CuestionarioID },
            dataType: 'json',
            ...
            error: function () {
                $.toast({ text: "Hubo un error al cargar los datos de la evaluación.", icon: "error", loader: true, position: 'top-right', allowToastClose: false });
            }
        });
        ...
        $('#add_group').click(function () {
            ...
            if (!grupoNombre) {
                $.toast({ text: "Por favor ingrese el nombre del grupo.", icon: "error", loader: true, position: 'top-right', allowToastClose: false });
                return;
            }
        ...
        $('#grupos_container').on('click', '.add_question', function () {
            ...
            const pregunta = { texto: '', tipo: 'multiple', respuestas: [], obligatoria: false, ponderacion: 0 };
        ...
        $('#save_evaluation').click(function () {
            ...
            $.ajax({
                ...
                type: "POST",
                dataType: 'json',
                data: { evaluation: JSON.stringify(evaluation), cuestionarioID: CuestionarioID ,deletedQuestions: JSON.stringify(deletedQuestions) },
                success: function () {
                    $.toast({ text: "Evaluación guardada exitosamente.", icon: "success", loader: true, position: 'top-right', allowToastClose: false });
                    location.reload();
                },
                error: function () {
                    $.toast({ text: "Hubo un error al guardar la evaluación.", icon: "error", loader: true, position: 'top-right', allowToastClose: false });
                }
            });
        });
                    ...
                    action: function() {
                        ...
                        if (seleccionados.length > 0) {
                            ...
                            $.ajax({
                                ...
                                type: "POST",
                                dataType: "json",
                                data: { ids: seleccionados , cuestionarioID: CuestionarioID},
                                success: function(response) {
                                    if (response.status === 'success') {
                                        $.toast({ text: response.message, icon: "success", loader: true, position: 'top-right', allowToastClose: false });
                                       
                                        tblEmpleados.ajax.reload(); // Recargar la tabla después de guardar
                                        tblCuestionarioEvaluados.ajax.reload();
                                    } else {
                                        $.toast({ text: response.message, icon: "error", loader: true, position: 'top-right', allowToastClose: false });
                                    }
                                },
                            ...
                            $.toast({ text: "Por favor, selecciona al menos un registro.", icon: "warning", loader: true, position: 'top-right', allowToastClose: false });
                        }
                    }
                }
            ],
        ...
        var tblCuestionarioEvaluados = $("#tblCuestionarioEvaluados").DataTable({
            ...
            columns: [
                ...
                { 
                    ...
                    render: function(data, type, row) {
                        return `
                            <button class="btn btn-danger btn-icon  btn-icon-mini btn-round btn-eliminar-evaluado" data-id="${row.eva_EvaluadoCuestionarioID}">
                                <i class="zmdi zmdi-delete"></i> 
                            </button>
                            <button class="btn btn-primary btn-icon btn-icon-mini btn-round btn-asignar-nivel" data-id="${row.eva_EvaluadoCuestionarioID}" data-emp="${row.emp_Nombre}">
