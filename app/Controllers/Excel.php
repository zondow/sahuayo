...
class Excel extends BaseController
{
    ...
    //Diego-> genera un excel con los departamentos
    public function generarExcelDepartamentos()
    {
        $spreadsheet = new Spreadsheet();
        $spreadsheet->getProperties()
            ->setCreator("Thigo")
            ->setTitle("Catalogo de departamentos")
            ->setSubject("Departamentos")
            ->setDescription("Catalogo de departamentos");
        ...
        $spreadsheet->getActiveSheet()->setCellValue('A1', 'NOMBRE DEL DEPARTAMENTO')
            ->setCellValue('B1', 'JEFE DEL DEPARTAMENTO')
            ->setCellValue('C1', 'EMPLEADOS DEL DEPARTAMENTO');

        $spreadsheet->getActiveSheet()->fromArray($resultado, '', 'A2');

        $spreadsheet->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getStyle('A1:C1')->getFont()->setBold(true);
        $spreadsheet->getActiveSheet()->getStyle('A1:C1')->getFont()->setName('Arial');
        $spreadsheet->getActiveSheet()->getStyle('A1:C1')->getFont()->setSize(10);
        $spreadsheet->getActiveSheet()->getStyle('A1:C1')->applyFromArray($estiloTitulo);
    ...
    public function generarExcelPuestos()
    {
        $spreadsheet = new Spreadsheet();
        $spreadsheet->getProperties()
            ->setCreator("Thigo")
            ->setTitle("Catalogo de puestos")
            ->setSubject("Puestos")
            ->setDescription("Catalogo de puestos");

        //$resultado = $this->db->query("select pue_Nombre,pue_Nivel from puesto where pue_Estatus=1 order by pue_Nombre ASC")->getResultArray();
        $resultado = $this->db->query("select pue_Nombre from puesto where pue_Estatus=1 order by pue_Nombre ASC")->getResultArray();
        ...
        $spreadsheet->getActiveSheet();
        $spreadsheet->setActiveSheetIndex(0);
        $spreadsheet->getActiveSheet()->setCellValue('A1', 'NOMBRE DEL PUESTO');

        $spreadsheet->getActiveSheet()->fromArray($resultado, '', 'A2');

        $spreadsheet->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getStyle('A1:A1')->getFont()->setBold(true);
        $spreadsheet->getActiveSheet()->getStyle('A1:A1')->getFont()->setName('Arial');
        $spreadsheet->getActiveSheet()->getStyle('A1:A1')->getFont()->setSize(10);
        $spreadsheet->getActiveSheet()->getStyle('A1:A1')->applyFromArray($estiloTitulo);
    ...
    public function generarExcelCooperativas()
    {
        $spreadsheet = new Spreadsheet();
        $spreadsheet->getProperties()
            ->setCreator("Thigo")
            ->setTitle("Catalogo de cooperativas")
            ->setSubject("Cooperativas")
            ->setDescription("Catalogo de cooperativas");


        $model = new ExcelModel();
        $resultado = $model->getCooperativas();
        ...
        $writer = new Xlsx($spreadsheet);
        header("Content-Type: application/vnd.ms-excel");
        header('Content-Disposition: attachment; filename="Cooperativas.xls"');
        $writer->save('php://output');
        exit();
    }
