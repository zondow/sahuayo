...
class ReclutamientoModel extends Model
{
    ...
    //Diego -> Obtiene el listado de solicitudes de personal para autorizar
    ...
    public function getSolicitudPersonalByID($id)
    {
        return $this->db->query("SELECT * FROM solicitudpersonal WHERE sol_SolicitudPersonalID=?", [$id])->getRowArray();
    }

    //Diego -> obtener info empleado
    function getEmpleadoInfoRequisicionPersonalID()
    {
        ...
        JOIN sucursal ON suc_SucursalID = emp_SucursalID 
        WHERE emp_EmpleadoID=" . session('id'))->getRowArray();
    } //end getEmpleadoInfoID

    //Diego -> obtener solicitudes eprsonal autorizadas
    function getListSolicitudesPersonalAutorizada()
    {
        return $this->db->query("SELECT * FROM solicitudpersonal 
        JOIN empleado ON emp_EmpleadoID=sol_EmpleadoID 
        LEFT JOIN puesto ON pue_PuestoID=sol_PuestoID 
        WHERE sol_DirGeneralAutorizada='AUTORIZADA' 
        ORDER BY sol_Estatus DESC, sol_Fecha DESC")->getResultArray();
    }
    ...
    function getSolicitudByID($solicitudID)
    {
        ...
        WHERE sol_SolicitudPersonalID=? 
        ORDER BY sol_SolicitudPersonalID DESC", [encryptDecrypt('decrypt', $solicitudID)])->getRowArray();
    }
    ...
    {
        return $this->db->query("SELECT *, '' as 'can_Num' 
        FROM candidato 
        JOIN solicitudpersonal ON sol_SolicitudPersonalID=can_SolicitudPersonalID 
        JOIN puesto ON pue_PuestoID=sol_PuestoID 
        WHERE can_Estatus='CARTERA'")->getResultArray();
    }

    //Diego -> obtener solicitudes eprsonal autorizadas y en proceso
    function getListSolicitudesPersonalActivas()
    {
        return $this->db->query("SELECT * FROM solicitudpersonal 
        JOIN empleado ON emp_EmpleadoID=sol_EmpleadoID 
        LEFT JOIN puesto ON pue_PuestoID=sol_PuestoID 
        WHERE sol_DirGeneralAutorizada='AUTORIZADA' AND sol_Estatus = 1 ORDER BY sol_Fecha DESC")->getResultArray();
    }
